...
## Forma Design System Local Agent Contract

Before creating, changing, or consuming Echelon UI with Forma, read [docs/agents/DESIGN-SYSTEM-USAGE.md](docs/agents/DESIGN-SYSTEM-USAGE.md).

Repository-specific rules:

1. Search the canonical `patterns/*.html` catalog and generated component showcase before inventing an application-local control.
2. Keep canonical components HTML/CSS only. The browser runtime budget for the design-system package is zero.
3. Preserve native semantics and accessibility structure. Do not replace fieldsets, legends, labels, native roles, keyboard paths, direct-entry alternatives, or special-state separation for visual convenience.
4. Keep meaningful/domain state in the application/Ordo layer. Use Limen only for browser behavior beyond native HTML.
5. Use package tokens and public CSS layers rather than copying values into consumer applications.
6. When adding a canonical pattern, add showcase metadata and tests so the generated site produces one component page with at least three examples.
7. Never edit `site-dist/` by hand. It is generated by `site/build-site.mjs`.
8. Run `npm run site:check` for showcase changes and the normal repository validation before completion.

The published Forma showcase is a discovery and teaching surface. Canonical source files and manifests remain authoritative.
